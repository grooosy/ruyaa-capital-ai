  ...
  const features = [
    {
      ...
      description: isArabic
        ? "يحلل ذكاء رؤيا مشاعر السوق من آلاف المصادر"
        : "Ruyaa AI analyzes market sentiment from thousands of sources",
    },
    {
      title: isArabic ? "إدارة المخاطر الذكية" : "Smart Risk Management",
      description: isArabic
        ? "حماية تلقائية لرأس المال مع تحديد الأحجام المثلى"
        : "Automatic capital protection with optimal position sizing",
    },
    {
    ...
    <Card className="bg-gradient-to-br from-card to-blue/5 border-blue/20">
      <CardHeader>
        <CardTitle className="flex items-center gap-3 text-white">
          <motion.div className="p-2 bg-blue/20 rounded-full" whileHover={{ scale: 1.1, rotate: 5 }}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="text-xs text-gray-400">{isArabic ? stat.labelAr : stat.label}</div>
            </motion.div>
          ))}
        </div>
