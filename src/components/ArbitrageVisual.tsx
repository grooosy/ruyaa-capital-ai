...
import { Zap, ArrowUpRight, DollarSign, Repeat } from 'lucide-react';

const ExchangeNode = ({ name, position, isBuy = false }: { name: string, position: string, isBuy?: boolean }) => (
    <div className={`absolute ${position}`}>
        <motion.div 
            className="w-32 h-32 rounded-full border border-green/20 bg-black/30 backdrop-blur-sm flex flex-col items-center justify-center"
        ...
        <motion.div 
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 p-3 rounded-lg bg-gray-900/50 border border-gray-700/50 backdrop-blur-md ${isBuy ? '-mt-24' : 'mt-24'}`}
...
const ArbitrageVisual = () => {
    ...
    return (
        <div className="w-full max-w-xl mx-auto aspect-square relative my-8">
            {/* Exchanges */}
            ...
            >
                <div className="w-36 h-36 rounded-full bg-green/10 flex items-center justify-center animate-pulse-subtle">
                    <div className="w-28 h-28 rounded-full bg-green/20 flex items-center justify-center">
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 500 500">
                ...
                {/* Buy Path */}
                <path d={path} fill="none" stroke="#16C784" strokeOpacity="0.2" strokeWidth="2" strokeDasharray="5 5" />
                {/* Sell Path */}
                <path d={reversePath} fill="none" stroke="#16C784" strokeOpacity="0.2" strokeWidth="2" strokeDasharray="5 5" />
             ...
             {/* Profit Node */}
            <motion.div 
                className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3 p-3 rounded-lg bg-gray-900/50 border border-gray-700/50 backdrop-blur-md"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
