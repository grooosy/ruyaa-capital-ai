...
const ProcessTimeline: React.FC<ProcessTimelineProps> = ({ timeline, theme }) => {
  return (
    <div className="relative flex flex-col md:flex-row md:items-start md:gap-0 gap-8 md:justify-between mt-4">
      ...
      <div 
        className={`hidden md:block absolute left-1/2 top-10 bottom-3 w-0.5 -translate-x-1/2 bg-gradient-to-b rounded-full ${theme === 'green' ? 'from-green/50 via-green/20' : 'from-gold/50 via-gold/20'} to-transparent`} 
        style={{ 
